...
exports.aiParserService = exports.AIParserService = void 0;
...
class AIParserService {
    ...
    geminiAI = null;
    constructor() {
        if (config_1.config.ai.geminiApiKey) {
            this.geminiAI = new generative_ai_1.GoogleGenerativeAI(config_1.config.ai.geminiApiKey);
        }
        if (config_1.config.ai.openaiApiKey) {
            this.openai = new openai_1.default({ apiKey: config_1.config.ai.openaiApiKey });
        }
    }
    /**
     * Main entry point: Parses text using Gemini / OpenAI if available, falling back to local NLP engine
     */
    async parseMessage(message, userTimezone = 'Asia/Kolkata') {
     ...
     */
    async parseWithGemini(text, userTimezone) {
        if (!this.geminiAI)
            return { status: 'FAILED', error: 'Gemini client not initialized' };
  ...
  "clarification": {
    ...
    "options": string[]
        ...
        const model = this.geminiAI.getGenerativeModel({
            model: config_1.config.ai.geminiModel || 'gemini-1.5-flash',
            generationConfig: {
     ...
     */
    async parseWithOpenAI(text, userTimezone) {
        if (!this.openai)
...
4. Output must strictly be valid JSON matching the following schema:
{
  ...
  "clarification": {
     ...
     */
    parseWithLocalNLP(text, _userTimezone = 'Asia/Kolkata') {
     ...
     */
    parseSingleClause(clause, defaultDate, defaultPaymentMethod) {
        ...
        const isAmbiguous = category === 'Other' &&
     ...
     */
    detectCategoryAndDetails(text, type) {
        const lower = text.toLowerCase();
     ...
     */
    cleanDescription(raw, matchedKeyword) {
        let desc = raw
    ...
    normalizePaymentMethod(pm) {
        ...
        if (upper === 'CARD')
            return 'CARD';
        if (upper === 'NET_BANKING' || upper === 'NETBANKING')
            return 'NET_BANKING';
        if (upper === 'WALLET')
            return 'WALLET';
        if (upper === 'CASH')
...
exports.AIParserService = AIParserService;
exports.aiParserService = new AIParserService();
