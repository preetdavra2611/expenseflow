    ...
    async getSummary(req, res, next) {
        try {
            ...
            for (const item of currentMonthTx) {
                if (item.type === 'INCOME')
                    currentMonthIncome = item._sum.amount || 0;
                if (item.type === 'EXPENSE')
                    currentMonthExpenses = item._sum.amount || 0;
            }
            let lastMonthExpenses = 0;
            for (const item of lastMonthTx) {
                if (item.type === 'EXPENSE')
                    lastMonthExpenses = item._sum.amount || 0;
            }
            const totalBalance = totalIncome - totalExpenses;
            const daysElapsed = Math.max(1, now.getDate());
            const avgDailySpending = Math.round(currentMonthExpenses / daysElapsed);
            const expenseChangePct = lastMonthExpenses > 0
    ...
    async getMonthly(req, res, next) {
        try {
            ...
            for (let i = 5; i >= 0; i--) {
                ...
                const totals = await config_1.prisma.transaction.groupBy({
                    by: ['type'],
                    where: {
                        userId,
