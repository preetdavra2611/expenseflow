...
export class AuthService {
  async register(email: string, password: string, name: string, currency: string = 'INR', timezone: string = 'Asia/Kolkata') {
  ...
  async getOrCreateTelegramUser(telegramUserId: string, telegramUsername?: string, name?: string) {
    ...
    if (cleanUsername) {
      const existingWebUser = await prisma.user.findFirst({
        where: {
          OR: [
  ...
  async linkTelegramAccount(userId: string, telegramUserId: string, telegramUsername?: string) {
    const cleanUsername = telegramUsername ? telegramUsername.replace(/^@/, '').trim().toLowerCase() : null;
    ...
    if (existingTemp && existingTemp.id !== userId) {
      ...
      const targetFoodCat = await prisma.category.findFirst({
        where: { userId, name: 'Food' },
      });

      const tempCats = await prisma.category.findMany({ where: { userId: existingTemp.id } });
      for (const tc of tempCats) {
        const matching = await prisma.category.findFirst({ where: { userId, name: tc.name } });
      ...
      await prisma.category.deleteMany({ where: { userId: existingTemp.id } });
  ...
  async updateSettings(userId: string, data: { name?: string; currency?: string; currencySymbol?: string; timezone?: string; telegramUsername?: string | null }) {
